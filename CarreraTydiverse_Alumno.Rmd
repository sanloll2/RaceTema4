...
```{r}
#Seleccionamos con función select.
select(cran, ip_id, package, country)
```
5. Elige todas las variables entre `r_arch y country` 
```{r}
#Establecemos : para que vaya de un extremo (r_arch) a otro (country)
...
```{r}
#Hacemos lo mismo que en el ejercicio anterior estableciendo un - para incidir 
#en todas las columnas menos de data-size
...
```{r}
#Empleamos tuberías para filtrar por el paquete dado y la versión establecida:
...
```{r}
#Hacemos lo mismo que en anteriores ejercicios:
...
```{r}
#Filtramos por la variable 'version' que no esté vacía:
cran %>% filter(!is.na(version))
```
10. Almacena en `cran2` todas las columnas entre `size` y `ip_id`. 
```{r}
#Seleccionamos del conjunto de datos cran dichas variables:
...
```{r}
#Hacemos un summarize para almacenarlo en pack_sum:
pack_sum <- by_package %>% summarize(count = n(), unique = n_distinct(ip_id), 
                                      countries = n_distinct(country), 
                                      avg_bytes = mean(size), 
...
#Variable de interés: 
#umbral <- cran %>% quantile(pack_sum$count, probs=0.99)
